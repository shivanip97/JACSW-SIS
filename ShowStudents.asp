...
<link rel="stylesheet" href="css/tabstyle.css" type="text/css" />
	...
	<script type="text/javascript">
	    ...
	    function getval(sel) {
	        window.location = "https://socialwork.cc.uic.edu/SIS/ShowStudents.asp?ID=" + sel.value;
	    }
 	</script>
    <style type="text/css">
		table {
...
<label style="font-size: 1.17em;font-weight: bold;">MSW Applicants -  </label><h3 id="termname" style="display:inline";></h3><br />
     ...
     <%
                        query = "select Term_CD,Admit_Term from AdmitTerm_Codes ORDER BY Admit_Term DESC"
    ...
    <tbody>
     <%
					...
					course_query="select * from Applicants where term_cd like '" & Request("ID") & "' order by LastName"
		...
		<tr>
            ...
            <td align="center"><div> <% Response.write rs("program_type") %></div></td>
             <td><button type="submit" name="Button1" onclick="this.form.action='ViewStudent.asp?ID=<%Response.write rs("term_cd") %>'; this.forms.submit();" id="Button1" value='<% Response.write rs("UIN") %>'>View/Edit</button></td>
         </tr>
		 <% rs.MoveNext   
        Loop End If %>
	</tbody>
