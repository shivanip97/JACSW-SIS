...
pdf.ChapterTitle2("                       Admissions Report - "&Termsel& " - Denial   "&LastUpdatedDt&" "&LastUpdatedTime)
...
set rs=Server.CreateObject("ADODB.recordset")
phd_query="SELECT Count(distinct UIN) phd_students FROM PHDApplicants where Degree_Program='PHD' and  Admission_decision in ('D') and Term_CD like '"&AdmitTerm&"' "
...
pdf.SetFont "Arial","",10
					...
					rs.close
					set rs=Server.CreateObject("ADODB.recordset")
					phd1_query="SELECT UIN, Firstname, LastName FROM Applicants where Degree_Program='PHD' and Admission_decision in ('D') and term_cd='"&AdmitTerm&"'"
...
pdf.GreyTitle("")
denied_students = "Total Students Denied : "&phd_rows
